    ...
    std::unique_ptr<State> state)
{
    ...
    // has e^w.
    firstBucketLogLoad_ = -intrinsicGrowthRate_ * state_->intParms.deltaTime *
        state_->intParms.deltaTime * state_->intParms.numInfectionLoadBuckets;
    columnLoads_ = std::make_unique<std::vector<double>>();
    columnLoads_->resize(state_->intParms.numInfectionLoadBuckets);
    ...
    for (size_t i = 2; i < state_->intParms.numInfectionLoadBuckets; i++) {
        columnLoads[i] = exp(firstBucketLogLoad_ + (i - 1) * state_->intParms.deltaTime *
...
void Integrator::findSums() {
    ...
    for (size_t ageIndex = 0; ageIndex < maxAgeStep; ageIndex++) {
        ...
        for (size_t infectionIndex = 1; infectionIndex < state_->intParms.numInfectionLoadBuckets; infectionIndex++) {
            ...
            transferRate_ += state_->modParms.beta * columnLoads[infectionIndex] * popAtLoad;
        }
    }
}
